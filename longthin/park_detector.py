...
class ParkDetector:
    def __init__(
        ...
        hue_limit_orange = np.deg2rad(-15), np.deg2rad(+30)
        hue_limit_blue = np.deg2rad(150), np.deg2rad(270)
        self.bounds_orange = calculate_lch_bounds(
            hue_limit_orange, chrome_orange, intensity_orange)
        self.bounds_blue = calculate_lch_bounds(
            hue_limit_blue, chroma_blue, intensity_blue)
